...
function showForm() {
  ...
  if (currentIndex >= shuffledIds.length) {
    container.innerHTML = '<h2>Thank you for completing all forms!</h2><button onclick="location.reload()">Return to home page</button>';
    return;
  }
  ...
  container.innerHTML = `
    <h2>Form ${currentIndex + 1}</h2>
    <iframe src="https://docs.google.com/forms/d/e/${formId}/viewform?embedded=true" width="640" height="1000" frameborder="0" marginheight="0" marginwidth="0">Loading...</iframe>
    <button onclick="showForm()">Next Form</button>
  `;
